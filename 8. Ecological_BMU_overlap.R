...
ref_month <- "01" # used once to build the common domain mask
...
# species HSI folders used to define the common domain
surfclam_hsi_dir <- "../results/HSI/surfclam/HSI_weighted"
scallop_hsi_dir <- "../results/HSI/scallop/HSI_weighted"
...
nearest_bmu <- function(x, codebook) {
  d2 <- outer(rowSums(x^2), rowSums(codebook^2), FUN = "+") -
    2 * x %*% t(codebook)
  list(
...
# ============================================================ #
# 3. Build aggregated tow dataset and fit SOM ----
# ============================================================ #
...
bmu_table <- tow |>
  ...
  mutate(
    ...
    n_quahog = replace_na(n_quahog, 0),
...
tow <- tow |>
  left_join(
    bmu_table |>
      select(BMU, BMU_order, BMU_label, temp_class, depth_class, sed_class),
    by = "BMU"
  )
...
# ============================================================ #
# 4. Prepare common rasters and common domain mask ----
# ============================================================ #

# sediment
sediment_nefsc <- rast(sediment_file)
sediment_vims <- rast(file.path(
  dirname(sediment_file),
  paste0("VIMS_", basename(sediment_file))
))
if (!same.crs(sediment_nefsc, sediment_vims)) {
  sediment_vims <- project(sediment_vims, sediment_nefsc)
}

sediment_vims <- resample(sediment_vims, sediment_nefsc, method = "bilinear")
sediment_rast <- cover(sediment_nefsc, sediment_vims)

# depth
depth_nefsc <- rast(depth_file)
depth_vims <- rast(file.path(
  dirname(depth_file),
  paste0("VIMS_", basename(depth_file))
))
if (!same.crs(depth_nefsc, depth_vims)) {
  depth_vims <- project(depth_vims, depth_nefsc)
}

depth_vims <- resample(depth_vims, depth_nefsc, method = "bilinear")
depth_rast <- cover(depth_nefsc, depth_vims)

# temperature
temp_ref_nefsc <- rast(file.path(
  temp_dir,
  paste0("bottomT_insitu_", year_tag, ref_month, "_1km_domain.tif")
))

temp_ref_vims <- rast(file.path(
  temp_dir,
  paste0("VIMS_bottomT_insitu_", year_tag, ref_month, "_1km_domain.tif")
))

if (!same.crs(temp_ref_nefsc, temp_ref_vims)) {
  temp_ref_vims <- project(temp_ref_vims, temp_ref_nefsc)
}

temp_ref_vims <- resample(temp_ref_vims, temp_ref_nefsc, method = "bilinear")
temp_ref_rast <- cover(temp_ref_nefsc, temp_ref_vims)

compareGeom(sediment_rast, depth_rast, temp_ref_rast, stopOnError = TRUE)

# load one reference-month HSI raster for each species
surfclam_ref <- rast(file.path(
  surfclam_hsi_dir,
  paste0("surfclam_HSI_weighted_", year_tag, ref_month, ".tif")
))

scallop_ref <- rast(file.path(
  scallop_hsi_dir,
  paste0("scallop_HSI_weighted_", year_tag, ref_month, ".tif")
))

# put both species masks onto the SAME master grid
# use nearest-neighbor because these are just masks / categorical support
surfclam_mask <- resample(surfclam_ref, sediment_rast, method = "near")
scallop_mask <- resample(scallop_ref, sediment_rast, method = "near")

# common domain = any cell covered by either surfclam or scallop
domain_mask <- sediment_rast
values(domain_mask) <- ifelse(
  !is.na(values(surfclam_mask)) | !is.na(values(scallop_mask)),
  1,
  NA
)
names(domain_mask) <- "domain_mask"

# WEA polygon and WEA raster on the same common grid
wea_plot <- st_read(wea_shp, quiet = TRUE) |>
  st_make_valid() |>
  st_transform(crs(sediment_rast))

bbox_sf <- st_as_sf(as.polygons(ext(sediment_rast), crs = crs(sediment_rast)))
...
names(wea_rast) <- "in_WEA"

compareGeom(domain_mask, wea_rast, stopOnError = TRUE)

# plot to double check the domain covers both surveys
plot(domain_mask, main = "Common domain mask")
plot(st_geometry(wea_plot), add = TRUE, border = "red", lwd = 0.8)


# ============================================================ #
# 5. Project BMUs to monthly rasters ----
# ============================================================ #
...
for (i in seq_along(months)) {
  ...
  # load and fix temperature raster
  temp_nefsc <- rast(file.path(
    temp_dir,
    paste0("bottomT_insitu_", year_tag, mm, "_1km_domain.tif")
  ))

  temp_vims <- rast(file.path(
    temp_dir,
    paste0("VIMS_bottomT_insitu_", year_tag, mm, "_1km_domain.tif")
  ))

  if (!same.crs(temp_nefsc, temp_vims)) {
    temp_vims <- project(temp_vims, temp_nefsc)
  }

  temp_vims <- resample(temp_vims, temp_nefsc, method = "bilinear")
  temp_rast <- cover(temp_nefsc, temp_vims)
  ...
  env_df <- c(sediment_rast, depth_rast, temp_rast, domain_mask) |>
    `names<-`(c("SED_PHI_MEAN", "DEPTH", "TEMP", "domain_mask")) |>
...
# ============================================================ #
# 6. Monthly and yearly BMU-WEA overlap / enrichment ----
# ============================================================ #
...
for (i in seq_along(months)) {
  ...
  compareGeom(bmu_rast, wea_rast, stopOnError = TRUE)
  ...
  monthly_counts <- overlay_df |>
    ...
    pivot_wider(names_from = in_WEA, values_from = n_cells, values_fill = 0) |>
    rename(
...
# ============================================================ #
# 7. Combined 12-month BMU map figure ----
# ============================================================ #
...
bmu_legend_df <- bmu_table |>
  arrange(BMU_order) |>
  mutate(legend_label = paste0("BMU ", BMU_order, ": ", BMU_label))

map_df_all <- map_df_all |>
  left_join(
    bmu_legend_df |>
      select(BMU, BMU_order, legend_label),
    by = c("BMU", "BMU_order")
  ) |>
  mutate(
    month = factor(month, levels = months),
    legend_label = factor(legend_label, levels = bmu_legend_df$legend_label)
  )
