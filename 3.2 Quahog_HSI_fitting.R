...
data <- read.csv("../data/quahog data/OQ_tows_MA_final.csv") |>
  filter(YR >= 2013) |> # this is more recent and in August
  filter(
    !is.na(NperMsq) & !is.na(TEMP) & !is.na(DEPTH) & !is.na(SED_PHI_MEAN)
  )
...
# fit SI across bin midpoints
sed_bell <- fit_bell_SI(SI_df = sediment_SI, SI_col = SI_sediment)

# predict SI_sediment across bins and re-scale to 0-1
sediment_SI <- sediment_SI |>
  mutate(
    SI_sediment = predict(sed_bell, newdata = sediment_SI)
  ) |>
  mutate(
    SI_sediment = {
...
saveRDS(sediment_SI, "../results/HSI/quahog/quahog_sediment_SI.rds")

remove(sediment, sed_brks, sediment_SI, sed_bell)
...
# fit SI across bin midpoints
depth_bell <- fit_bell_SI(SI_df = depth_SI, SI_col = SI_depth)

# predict SI_depth across bins and re-scale to 0-1
depth_SI <- depth_SI |>
  mutate(
    SI_depth = predict(depth_bell, newdata = depth_SI)
  ) |>
  mutate(
    SI_depth = {
...
saveRDS(depth_SI, "../results/HSI/quahog/quahog_depth_SI.rds")

remove(depth, depth_brks, depth_SI, depth_bell)
...
# fit SI across bin midpoints
temp_bell <- fit_bell_SI(SI_df = temp_SI, SI_col = SI_temp)

# predict SI_temp across bins and re-scale to 0-1
temp_SI <- temp_SI |>
  mutate(
    SI_temp = predict(temp_bell, newdata = temp_SI)
  ) |>
  mutate(
    SI_temp = {
...
saveRDS(temp_SI, "../results/HSI/quahog/quahog_temp_SI.rds")

remove(temp, temp_brks, temp_SI, temp_bell)
...
write.csv(
  data,
  "../results/HSI/quahog/OQ_tows_with_separate_SI.csv",
  row.names = FALSE
)
...
data <- read.csv("../results/HSI/quahog/OQ_tows_with_separate_SI.csv") # load seperate SI
...
plot(brt_fit)
brt_fit$contributions # 0.01 - 39, 33, 28
brt_fit$n.trees
...
write.csv(
  brt_weights,
  "../results/HSI/quahog/quahog_BRT_weights.csv",
  row.names = FALSE
)
write.csv(
  data,
  "../results/HSI/quahog/OQ_tows_with_composite_SI.csv",
  row.names = FALSE
)
