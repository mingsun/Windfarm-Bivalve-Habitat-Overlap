...
depth_rast <- rast("../data/bathymetry data/VIMS_depth_gebco_1km_domain.tif")
...
sediment_SI <- readRDS("../results/HSI/scallop/scallop_sediment_SI.rds")
depth_SI <- readRDS("../results/HSI/scallop/scallop_depth_SI.rds")
temp_SI <- readRDS("../results/HSI/scallop/scallop_temp_SI.rds")

# ------------------------------------------------------------ #

# 3. Load BRT weights ----

brt_weights <- read.csv("../results/HSI/scallop/scallop_BRT_weights.csv")
...
## Combine and save extrapolation summary ----
extrap_df <- bind_rows(extrap_list)

write.csv(
  extrap_df,
  "../results/HSI/scallop/HSI_weighted/temperature_extrapolation_summary_2025.csv",
  row.names = FALSE
)
