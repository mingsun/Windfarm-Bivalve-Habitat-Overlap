...
catch_by_tow_df <- read.csv("../data/surfclam data/SCtows_original.csv") |>
  filter(STRATUM %in% paste0(1:6, "S")) |> # filter out the SVAtoSNE region
...
write.csv(
  catch_by_tow_df,
  "../data/surfclam data/SC_tows_MA.csv",
  row.names = FALSE
)
...
overlay.df <- read.csv("../data/surfclam data/Surfclam_tow_overlap.csv") |>
...
write.csv(
  catch_by_tow_WEE_df,
  "../data/surfclam data/SC_tows_MA_WEE.csv",
  row.names = FALSE
)
...
png(
  "../plot/appendix/surfclam_effort_loss.png",
  width = 10,
  height = 3,
  units = 'in',
  res = 800
...
png(
  "../plot/appendix/surfclam_sample_loss.png",
  width = 10,
  height = 3,
  units = 'in',
  res = 800
