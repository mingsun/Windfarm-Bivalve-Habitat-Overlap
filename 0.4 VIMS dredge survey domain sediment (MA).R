...
# 1. Read tow data ----
station_df <- read.csv("../data/scallop data/VIMS_scallop_survey.csv") |>
  filter(beglon <= -69, beglat <= 41.7) |>
  select(YEAR = Year, LON = beglon, LAT = beglat) # drop rows with missing coordinates
...
st_write(
  st_as_sf(dom_buf),
  "../data/sediment data/VIMS_survey_domain_polygon.gpkg",
  delete_dsn = TRUE,
  quiet = TRUE
)
